...
import { getRanking, upsertRankEntry, deleteRankEntry } from '../../lib/supabase'
  ...
  return (
    <div style={{ flex:1, overflowY:'auto', padding:14 }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
        ...
        <button className="btn btn-p btn-sm" onClick={()=>{setEditEntry(null);setShowAdd(true)}}>+ Adicionar</button>
      </div>
      <div style={{ fontSize:11, color:'var(--muted)', marginBottom:14 }}>Classificação por pontuação · atualize manualmente conforme o progresso do servidor</div>

      {ranking.length === 0 && (
        <div style={{ textAlign:'center', padding:32, color:'var(--muted)', fontSize:12 }}>
          <div style={{ fontSize:36, marginBottom:8 }}>🏆</div>
          Sem entradas no ranking. Clique em "+ Adicionar".
        </div>
      )}
      ...
      {top3.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1.1fr 1fr', gap:10, marginBottom:14, alignItems:'end' }}>
          {podiumOrder.map((e,pi)=>{
            ...
            return (
              <div key={e.id} onDoubleClick={()=>{setEditEntry(e);setShowAdd(true)}} style={{ background:'var(--card)', border:`1px solid ${isFirst?'rgba(255,179,0,.35)':'var(--border)'}`, borderRadius:9, padding: isFirst?'18px 12px':'14px 10px', textAlign:'center', cursor:'pointer' }}>
                <span style={{ fontSize:isFirst?32:28, display:'block', marginBottom:7 }}>{medals[pi]}</span>
                <Avatar name={e.player_name} color={e.color} size={isFirst?58:48} style={{ margin:'0 auto 7px' }} />
                <div style={{ fontFamily:'Bangers,cursive', fontSize:isFirst?17:14, letterSpacing:1 }}>{e.player_name}</div>
                <div style={{ fontSize:9, color:'var(--muted)', marginBottom:5 }}>{e.char_name}</div>
                <div style={{ fontFamily:'Orbitron,monospace', fontSize:isFirst?16:13, fontWeight:700, color:'var(--gold)' }}>{e.points}</div>
              </div>
            )
          })}
        </div>
      ...
      {rest.length > 0 && (
        <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
          {rest.map((e,i)=>(
            <div key={e.id} onDoubleClick={()=>{setEditEntry(e);setShowAdd(true)}} style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:7, padding:'9px 12px', display:'flex', alignItems:'center', gap:10, cursor:'pointer', transition:'all .2s' }}
              onMouseEnter={ev=>ev.currentTarget.style.borderColor='var(--glow)'} onMouseLeave={ev=>ev.currentTarget.style.borderColor='var(--border)'}>
              <div style={{ fontFamily:'Orbitron,monospace', fontSize:12, fontWeight:700, color:'var(--dim)', width:22, textAlign:'center' }}>{i+4}</div>
              <Avatar name={e.player_name} color={e.color} size={34} />
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontFamily:'Rajdhani,sans-serif', fontWeight:700, fontSize:13 }}>{e.player_name}</div>
                <div style={{ fontSize:10, color:'var(--muted)' }}>{e.char_name}{e.quirk_name?' · '+e.quirk_name:''}</div>
              </div>
              {e.rank_badge && <div style={{ fontSize:8, fontWeight:700, padding:'2px 5px', borderRadius:3, background:'rgba(37,99,235,.2)', color:'var(--blue-l)', border:'1px solid rgba(37,99,235,.3)' }}>{e.rank_badge}</div>}
              <div style={{ fontFamily:'Orbitron,monospace', fontSize:12, color:'var(--gold)' }}>{e.points}</div>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <Modal title={entry?'✏️ Editar Entrada':'+ Ranking'} onClose={onClose}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
        ...
        <div className="field"><label>Pontuação</label><input className="input" type="number" value={form.points} onChange={e=>set('points',e.target.value)} /></div>
